...
const StatCard = ({ icon: Icon, label, value, color, secondaryLabel }: any) => (
  <div className="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-xl shadow-slate-200/20 group hover:-translate-y-1 transition-all duration-300">
    ...
    <div className="space-y-1">
      <div className="flex items-end gap-2">
        ...
        {secondaryLabel && <span className="text-[10px] font-black text-slate-400 uppercase mb-1.5">{secondaryLabel}</span>}
      </div>
...
const ProfileSection = ({ profile }: ProfileSectionProps) => {
  ...
  return (
    <div className="space-y-8">
      {/* Identity Card */}
      <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-2xl shadow-slate-200/30 relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-8 relative z-10">
          ...
          <div className="text-center sm:text-left flex-1">
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">{profile.full_name}</h2>
            <p className="text-slate-400 font-bold mt-1">{profile.email}</p>
