...
export const getProfile = async (req: Request, res: Response) => {
  try {
    ...
    const userResult = await pool.query(
      'SELECT id, email, phone_number, full_name, role, profile_image_url, settings FROM users WHERE id = $1',
      [userId]
    );
...
export const updateProfile = async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user?.id;
    const { full_name, phone_number, profile_image_url, settings } = req.body;
    ...
    const updateResult = await pool.query(
      `UPDATE users
       SET full_name = COALESCE($1, full_name),
           ...
           updated_at = NOW()
       WHERE id = $5
       RETURNING id, email, phone_number, full_name, role, profile_image_url, settings`,
      [full_name, phone_number, profile_image_url, settings, userId]
    );
